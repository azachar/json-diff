...
(function() {
  var colorize, diff, fs, sortJson, tty;
  ...
  sortJson = require('sort-json').sortJson;

  module.exports = function(argv) {
    var data1, data2, json1, json2, options, result;
    options = require('dreamopt')(["Usage: json-diff [-vjCks] first.json second.json", "Arguments:", "  first.json              Old file #var(file1) #required", "  second.json             New file #var(file2) #required", "General options:", "  -v, --verbose           Output progress info", "  -C, --[no-]color        Colored output", "  -j, --raw-json          Display raw JSON encoding of the diff #var(raw)", "  -k, --keys-only         Compare only the keys, ignore the differences in values #var(keysOnly)", "  -s, --sort              Sort both JSON files by keys, and sort primitive values in arrays before comparing"], argv);
    if (options.verbose) {
